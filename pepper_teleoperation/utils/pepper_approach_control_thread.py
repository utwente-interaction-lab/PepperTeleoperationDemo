...
def _scalar_rad(x):
    """Butterworth output may be ndarray; ensure a plain float for setAngles (Py2.7 safe)."""
    if x is None:
        return 0.0
    try:
...
class PepperApproachControl(Thread):
    ...
    def __init__(self, session, show_plot, approach_requested, approach_only, queue_in, queue_out, queue_motion=None):
        ...
        self.queue_motion = queue_motion
    ...
    def joints_control(self, session):
        ...
        # Kill Autonomous Life - without this Pepper fights every setAngles call.
        try:
            life_service = session.service("ALAutonomousLife")
            prev = life_service.getState()
            if prev != "solitary":
                life_service.setState("solitary")
            print("[Mirror] AutonomousLife -> solitary (was: %s)." % prev)
        except Exception as _e:
            print("[Mirror] AutonomousLife warning: %s" % _e)
        ...
        # Set stiffness of all relevant joints
        stiffness = 1.0
        for jnt in ["LShoulderPitch", "LShoulderRoll", "LElbowYaw", "LElbowRoll",
                    "RShoulderPitch", "RShoulderRoll", "RElbowYaw", "RElbowRoll",
                    "RWristYaw", "HipPitch"]:
            motion_service.setStiffnesses(jnt, stiffness)
        print("[Mirror] All arm stiffnesses set to 1.0.")
        
        # Disable external collision protection so arms can reach target poses
            ...
            try:  
                ...
                if not wp_dict:
                    ...
                    if (empty_frames % 30) == 0:
                        self.queue_out.put("No body keypoints yet (check tracker view / stand in frame).")
                    continue
                else:
                    if not had_keypoints:
                        self.queue_out.put("Keypoints received. Mirroring is active.")
                        had_keypoints = True
                    empty_frames = 0
                
                # Save received keypoints (only when plot export path exists)
                if path is not None:
                ...
                # Control angles list 
                angles = [_scalar_rad(self.LShoulderPitch), _scalar_rad(self.LShoulderRoll), _scalar_rad(self.LElbowYaw), _scalar_rad(self.LElbowRoll),
                          _scalar_rad(self.RShoulderPitch), _scalar_rad(self.RShoulderRoll), _scalar_rad(self.RElbowYaw), _scalar_rad(self.RElbowRoll), _scalar_rad(self.HipPitch)]

                ## Send control commands after filter init (short delay so first frames stabilize) ##
                ...
                # Telemetry for GUI/console feedback (throttled to ~1 Hz)
                if (self.time_elapsed - last_motor_log_t) >= 1.0:
                    lsp_s = memProxy.getData("Device/SubDeviceList/LShoulderPitch/Position/Sensor/Value")
                    lsr_s = memProxy.getData("Device/SubDeviceList/LShoulderRoll/Position/Sensor/Value")
                    rsp_s = memProxy.getData("Device/SubDeviceList/RShoulderPitch/Position/Sensor/Value")
                    rsr_s = memProxy.getData("Device/SubDeviceList/RShoulderRoll/Position/Sensor/Value")
                    hp_s  = memProxy.getData("Device/SubDeviceList/HipPitch/Position/Sensor/Value")

                    msg = (
                        "MOTORS tgt[LSP %.2f LSR %.2f RSP %.2f RSR %.2f HP %.2f] "
                        "sens[LSP %.2f LSR %.2f RSP %.2f RSR %.2f HP %.2f]"
                        % (angles[0], angles[1], angles[4], angles[5], angles[8],
                           lsp_s, lsr_s, rsp_s, rsr_s, hp_s)
                    )
                    print(msg)
                    self.queue_out.put(msg)
                    last_motor_log_t = self.time_elapsed
...
if __name__ == "__main__":
    ...
    _qi = _StdQueue()
    _qo = _StdQueue()
    pac = PepperApproachControl(
        session, show_plot, approach_requested, approach_only, _qi, _qo, None)
    pac.start()
